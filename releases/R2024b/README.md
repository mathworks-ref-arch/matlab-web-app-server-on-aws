...
|Assign Public IP to EC2 Instance Hosting MATLAB Web App Server | Specify whether the deployed EC2 instance must use a public IP address. If you select `No`, you must provide a private subnet for the parameter `Subnet for MATLAB Web App Server`. <p>**Note:** Even after you select `No`, your MATLAB Web App Server apps home page is still accessible over the Internet. However, you cannot remotely connect to the EC2 instance hosting the server from outside the VPC.</p> |
