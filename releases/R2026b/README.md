# MATLAB Web App Server on Amazon Web Services - R2026b

# Deployment Steps
Follow these steps to deploy the R2026b MATLAB Web App Server reference architecture on AWS. To deploy reference architectures for other releases, see [Deploy Reference Architecture for Your Release](/README.md?tab=readme-ov-file#deploy-reference-architecture-for-your-release).
...
|---------|---------| ------------ | ----------------- |
| R2026b | <a href="https://us-east-1.console.aws.amazon.com/cloudformation/home?region=us-east-1#/stacks/quickcreate?templateURL=https://matlab-web-app-server-templates.s3.amazonaws.com/r2026b_refarch/WebAppServer_new.yml" target="_blank">     <img src="https://s3.amazonaws.com/cloudformation-examples/cloudformation-launch-stack.png"/> </a> | <a  href ="https://us-east-1.console.aws.amazon.com/cloudformation/home?region=us-east-1#/stacks/create/review?templateURL=https://matlab-web-app-server-templates.s3.amazonaws.com/r2026b_refarch/WebAppServer_existing.yml"  target ="_blank" >      <img  src ="https://s3.amazonaws.com/cloudformation-examples/cloudformation-launch-stack.png" /> </a> | Windows Server 2022 or Ubuntu 24.04 VM |
...
1. Provide values for parameters in the **Create Stack** page:
    ...
    | **ARN of SSL Certificate** | Specify the Amazon Resource Name (ARN) of the SSL certificate you uploaded to the **AWS Certificate Manager**. The ARN identifies the ACM certificate that encrypts client traffic to the load balancer. To retrieve an ARN, type **"Certificate Manager"** in the AWS console search box at the top of the page, and open the **AWS Certificate Manager** in a new tab.<p><em>*Example*</em>: `arn:aws:acm:us-east-1:012345678910:certificate/666abcd6-ab6c-6ab6-a666-a666666bcd66`</p> | 
...
1. To view the security group details, look for the **Logical ID** named `SecurityGroup` and click the corresponding URL listed under **Physical ID**.
...
1. On Linux servers, user authentication to the server home page is enabled by default through [Keycloak](https://www.keycloak.org/docs/latest/server_admin/index.html). 
    ...
    There are three pre-configured user accounts in Keycloak that you can use to log in to the server home page: `matlab-webapps-admin`, `matlab-webapps-author`, and `matlab-webapps-user`. After you log in, you see the `SampleApps` folder, which is created by default and contains a sample web app. 

    |User account |&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Permissions | &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Group Membership |
    |-|-|-|
    |`matlab-webapps-admin` | <ul><li>Access the admin portal</li><li>Upload, delete, and execute web apps in the `SampleApps` folder</li><li>Access the server home page</li></ul> | <ul><li>**MATLAB Web App Server Administrators**</li><li>**MATLAB Web App Server Authors**</li><li>**MATLAB Web App Server Users**</li></ul>|
    |`matlab-webapps-author` | <ul><li>Upload, delete, and execute web apps in the `SampleApps` folder</li><li>Access the server home page</li></ul>| <ul><li>**MATLAB Web App Server Authors**</li><li>**MATLAB Web App Server Users**</li></ul> |
    |`matlab-webapps-user` | <ul><li>Execute web apps in the `SampleApps` folder</li><li>Access the server home page</li></ul>| <ul><li>**MATLAB Web App Server Users**</li></ul> |
...
## Update IP Address Access for Existing Deployment
To allow new IP addresses to access an existing MATLAB Web App Server deployment, update the security group inbound rules.

1. In the AWS management console, select the nested stack named `MatlabWebAppServerResourcesStack`. If it is not present in the stack list, ensure the **View nested** option is enabled.
1. In the *Stack details* for the nested stack, click **Resources**.
1. Update the following security groups based on what access you need to change:

   **To change remote access (SSH/RDP) to the EC2 instance:**
   1. To view the security group details, look for the **Logical ID** named `MatlabWebappServerCloudStackSg` and click the corresponding URL listed under **Physical ID**.
   1. Update the source IP address range in CIDR notation for the following inbound rules:
      - Port 22 (SSH for Linux)
      - Port 3389 (RDP for Windows)
   1. Save the changes.

   **To change web access (apps home page and admin portal):**
   1. To view the security group details, look for the **Logical ID** named `MatlabWebappServerCloudStackSg2` and click the corresponding URL listed under **Physical ID**.
   1. Update the source IP address range in CIDR notation for the following inbound rules:
      - Port 443 (HTTPS)
      - Port 80 (HTTP)
   1. Save the changes.
...
The deployment generates self-signed SSL/TLS certificates for MATLAB Web App Server, Keycloak authentication, and admin portal services in three locations on the EC2 instance. The EC2 certificates handle TLS between the ALB and the backend services. For more details, including certificate requirements, see [Enable SSL on MATLAB Web App Server](https://www.mathworks.com/help/webappserver/ug/enable-ssl.html) in the MathWorks documentation.

The Application Load Balancer (ALB) front-end certificate is specified using the **ARN of SSL Certificate** template input parameter. It encrypts client traffic to the load balancer. Typically, you replace the ALB certificate if you own a domain name and want to avoid browser security warnings.

### Replace ALB Certificate
1. In the AWS management console, navigate to **EC2** > **Load Balancers**.
1. Select the load balancer associated with your stack.
1. Select the **Listeners** tab and click the current SSL certificate.
1. Replace the certificate with your updated version.

>**Note (Linux only):** If you replace the ALB certificate, you must also replace the Keycloak and admin portal certificates on the EC2 instance. The SAN of the new ALB certificate must include the SAN of each EC2 certificate. For details, see [Replace Keycloak Certificate](#replace-keycloak-certificate-linux-only).

### EC2 Certificate Locations
| Location | Purpose | CN | SAN | Default Expiration | CA file required |
|----------|---------|----|-----|--------------------|------------------|
| Linux: `/MathWorks`<br>Windows: `C:\MathWorks` | Web App Server SSL | `webapps.com` | N/A | 10 years | No. To enable browser trust or an external load-balancer handshake, import a copy of the public certificate as the CA file. |
| `/MathWorks/Keycloak/data/tls` | Keycloak authentication (Linux only) | ALB public DNS name | `localhost`, VM private DNS name | 1 year | Yes. The SAN of the ALB front-end certificate must include the SAN of this certificate. If using a private/internal CA, first add your root CA certificate to the system trust store. If your root CA is already in the system trust store, copy the public certificate to the corresponding `.ca` file name. |
| `/local/MathWorks/webapps/latest/config/webapps_private` | Admin portal services (Linux only) | ALB public DNS name | `localhost`, VM private DNS name | 5 years | Yes. The SAN of the ALB front-end certificate must include the SAN of this certificate. If using a private/internal CA, first add your root CA certificate to the system trust store. |

### Replace EC2 Certificates
1. Connect to the EC2 instance. For details, see [Connect to EC2 Instance Hosting MATLAB Web App Server Using SSH](#connect-to-ec2-instance-hosting-matlab-web-app-server-using-ssh).
...
1. Connect to the EC2 instance using SSH. For details, see [Connect to EC2 Instance Hosting MATLAB Web App Server Using SSH](#connect-to-ec2-instance-hosting-matlab-web-app-server-using-ssh).
...
1. Re-issue that `docker create` command, replacing the value of `-e KC_HOSTNAME=<current-value>` with the ALB public DNS name for your deployment.
...
| `8080`, `5000`, `5001`, `3000`, `9090` | Required for admin portal services. |
| `8443`, `9000` | Required for Keycloak services. |
